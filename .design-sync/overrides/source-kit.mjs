// forked from design-sync lib/source-kit.mjs — synth-entry barrel now re-exports default exports as named so
// default-exported components (Marquee, PageHero, ClosingCTA, etc.) appear on window.ImaginaryCakes
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const DEFAULT_EXPORT_RX = /export\s+default\s+(?:function|class)\s+([A-Z][A-Za-z0-9]*)/;
    const lines = comps.flatMap((p) => {
      const content = readFileSync(p, 'utf8');
      const defaultMatch = DEFAULT_EXPORT_RX.exec(content);
      const result = [`export * from ${JSON.stringify(p)};`];
      if (defaultMatch) result.push(`export { default as ${defaultMatch[1]} } from ${JSON.stringify(p)};`);
      return result;
    });
