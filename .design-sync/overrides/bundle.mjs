// forked from design-sync lib/bundle.mjs — Next.js inlines process references; inject polyfill banner
...
export async function bundleToIife({ entry, globalName, nodePaths, out, tsconfig }) {
  ...
  try {
    buildResult = await build({
      ...
      // Next.js bundles reference process at runtime; define a polyfill so the
      // IIFE loads correctly in a browser preview without a Node.js environment.
      banner: { js: 'var process=globalThis.process||(globalThis.process={env:{NODE_ENV:"production"},browser:true,version:""});' },
      outfile: bundleJs,
      logLevel: 'warning',
